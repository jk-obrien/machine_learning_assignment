...
There seems to be a lot of skew in some variables (e.g. gryos_dumbbell_y) but if
we use the random forest algorithm, our approach is not sensitive to skew and as 
there seem to be no other obvious problems we will not apply any transformations 
to the data.
...
This gives us a value of `r round(oos*100, 4)`% for our OOS error, which is reasonably 
close to the OOB error.
